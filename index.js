...
let replaceName = artists.filter(obj => obj.name.includes("Vincent van Dough")).map(obj => obj.name = "Vincent Van Gogh");
...
If correct, the function should return ["Salvador Dali", "Frida Kahlo"]*/
  ...
  function get20s(array){
    ...
    var lowEnd = years[0];
    var highEnd = years[1];
    return lowEnd >= 1900 && highEnd <= 2000;
    }).map(obj => obj.name)
  }
...
function addArtist(array){
  let newObj = {
      ...
      bio: 'Im transitioning from fitness to tech.'
  };
  array.push(newObj);  
  return array;
}
...
function lotsOfArt(array){
  let newArray = array.filter(obj => obj.paintings > 100).map(obj => obj.name);
  return newArray;
}
